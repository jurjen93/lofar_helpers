...
# parallel deconvolution and deconvolution-channels options
...
def phaseup(msinlist,msoutlist,datacolumn='CORRECTED_DATA'):
  msoutlist = []
  for ms in msinlist:
    msout=ms + '.phaseup'
    ...
    cmd += "add.type=stationadder add.stations={ST001:'CS*'}"
    cmd += "filter.type=filter filter.baseline='!CS*&&*' filter.remove=True"
    print(cmd)
    os.system(cmd)
...
def makeBBSmodelforTGSS(boxfile, fitsimage=None):
...
def beamcor(ms, usedppp=True):
    ...
    if usedppp and not phasedup :
        cmddppp = 'DPPP numthreads=32 msin=' + ms + ' msin.datacolumn=DATA msout=. '
...
def applycalold(ms, parmdb, soltype, preapplyphase=False, preapplyslow=False, TEC=False, rotation=False, puretec=False):
    ...
    if (soltype == 'complexgain' or soltype == 'rotation+diagonal' or soltype == 'scalaramplitude' \
      ...
      if rotation:
        ...
        if soltype == 'scalaramplitude' or soltype == 'amplitudeonly' or soltype == 'fulljones':
          cmd += 'steps=[ac2] '  
        else:    
          cmd += 'steps=[ac1,ac2] '
    ...
    if (soltype == 'complexgain' or soltype == 'rotation+diagonal' or soltype == 'scalaramplitude' or \
      ...
      if TEC == False:
        if rotation:  
         cmd += 'msout.datacolumn=CORRECTED_DATA steps=[ac0,ac1,ac2,ac3] '
          ...
          if soltype == 'scalaramplitude' or soltype == 'amplitudeonly' or soltype == 'fulljones':
            cmd += 'msout.datacolumn=CORRECTED_DATA steps=[ac0,ac2] '  
          else:
        ...
        cmd += 'ac0.correction=phase000 ac1.correction=phase000 '
        if soltype == 'fulljones':
          cmd += 'ac2.correction=fulljones '
          cmd += 'ac2.soltab=[phase000,amplitude000] '  
        else:
      ...
      if TEC == True:
        ...
        cmd += 'ac3.parmdb='+parmdb + ' ac3.type=applycal '
        if soltype == 'fulljones':
          cmd += 'ac3.correction=fulljones '
          cmd += 'ac3.soltab=[phase000,amplitude000] '
        else:    
          cmd += 'ac3.correction=amplitude000 '
...
def makeimage(mslist, imageout, pixsize, imsize, channelsout, niter, robust, \
              ...
              removenegativecc=True, usewgridder=False, paralleldeconvolution=0, deconvolutionchannels=0):
...
def runDPPP(ms, solint_ap, solint_phaseonly, nchan_phase, nchan_ap, parmdb, soltype, soltypepert=None, 
             ...
             SMconstraintslow=0.0, SMconstraintslowpert=0.0, SMconstraintphase=0.0, smoothcal=True, puretec=False, restoreflags=False, \
             soltype_fastphase='scalarphase', maxiter=100, flagging=False, \
             antennaconstraint=None, antennaconstraintslow=None, \
    ...
    cmd = 'DPPP numthreads=32 ' + 'msin=' + ms + ' msin.datacolumn=' + incol + ' '
    ...
    if preapplyslow:
      ...
      # RUN DPPP again
      cmd = 'DPPP numthreads=32 ' + 'msin=' + ms + ' msin.datacolumn=CORRECTED_DATA_AMP msout=. '
      cmd += 'msin.weightcolumn='+weight_spectrum + ' '
      cmd += 'msin.modelcolumn=MODEL_DATA '
      cmd += 'steps=[ddecal] ' + 'msout.storagemanager=dysco ddecal.type=ddecal '
      cmd += 'ddecal.maxiter='+str(np.int(maxiter)) +' ddecal.tolerance=1.e-4 ddecal.propagatesolutions=True '
...
parser.add_argument('--pixelscale', help='pixels size in arcsec, default=3.0/1.5 (LBA/HBA)', type=float)
...
parser.add_argument('--autofrequencyaverage', help='by default frequency averaging is tried if it does not result in bandwidth  smearing (True/False, default=True)', type=eval, choices=[True, False], default='True')
...
if args['start'] == 0 and args['useaoflagger']:
  runaoflagger(mslist) 
  ...
  if (args['skymodel'] != None) and (i ==0):
    for msnumber, ms in enumerate(mslist):   
      ...
      runDPPP(ms, np.int(solint_ap[msnumber]), np.int(solint_phase[msnumber]), \
               ...
               usesourcedb=True, skymodel=args['skymodel'], smoothcal=args['smoothcal'], \
               flagslowphases=args['doflagslowphases'], flagslowamprms=args['flagslowamprms'], flagslowphaserms=args['flagslowphaserms'])
  ...
  for msnumber, ms in enumerate(mslist):
    ...
    if i < args['phasecycles']:      
      runDPPP(ms, np.int(solint_ap[msnumber]), np.int(solint_phase[msnumber]), \
               ...
               SMconstraintphase=args['smoothnessconstraint_phase'], puretec=args['pure_tec'], \
               antennaconstraint=antennaconstraint_phase)
    else:
      if args['solve_rotation'] == True:
        runDPPP(ms, np.int(solint_ap[msnumber]), np.int(solint_phase[msnumber]), \
               ...
               flagslowphaserms=args['flagslowphaserms'], preapplyslow=args['dosecondslow'])          
      else:
        runDPPP(ms, np.int(solint_ap[msnumber]), np.int(solint_phase[msnumber]), \
               np.int(nchan_phase[msnumber]), np.int(nchan_ap[msnumber]), \
