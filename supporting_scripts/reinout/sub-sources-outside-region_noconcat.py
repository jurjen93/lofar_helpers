...
parser.add_argument('--nopredict', help='Do not do predict step (for use if repeating last step in case of failure)',
                    action='store_true')
parser.add_argument('--onlyuseweightspectrum', help='Only use WEIGHT_SPECTRUM, ignore IMAGING_WEIGHT',
                    action='store_true')
parser.add_argument('--HMPmodelfits', help='if provided, use this HMP model fits  for predict')
parser.add_argument('--nosubtract', help='Do not do subtract step (for use if repeating last step in case of failure)',
                    action='store_true')
...
if dopredict:
    ...
    if holesfixed:
        ...
        if args['h5sols'] != None:
            if not args['useHMP']:
                run("DDF.py --Output-Name=image_dd_SUB --Data-ChunkHours=" + str(args['chunkhours']) + " --Data-MS=" +
                    args['mslist'] + " --Deconv-PeakFactor 0.001000 --Data-ColName " + args[
                ...
                run("DDF.py --Output-Name=image_dd_SUB --Data-ChunkHours=" + str(args['chunkhours']) + " --Data-MS=" +
                    args['mslist'] + " --Data-ColName " + args['column'] + " --Parallel-NCPU=" + str(
