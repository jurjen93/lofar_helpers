...
singularity exec -B ${SING_BIND} ${SING_IMAGE} --noprofile --norc python ${SCRIPT_FOLDER}/runwscleanLBautoR.py -b ${TO}/boxes/box_${i}.reg --auto box_${i}.dysco.sub.shift.avg.weights.ms.archive0.goodtimes
# singularity exec -B ${SING_BIND} ${SING_IMAGE} --noprofile --norc python ${SCRIPT_FOLDER}/runwscleanLBauto.py -b ${TO}/boxes/box_${i}.reg --forwidefield --smoothnessconstraint-slow=5.0 --usemodeldataforsolints --slow-soltype=complexgain --usewgridder --avgfreqstep=2 --avgtimestep=2 --imager=DDFACET --useaoflagger --noarchive box_${i}.dysco.sub.shift.avg.weights.ms.archive0.goodtimes
