...
OUTPUT_FOLDER=${FOLDER}/result
mkdir ${OUTPUT_FOLDER}

cd ${OUTPUT_FOLDER}

#converging .npz to .h5
#singularity exec -B ${SING_BIND} ${SING_IMAGE} --noprofile --norc killMS2H5parm.py lotss_merged.h5 ${FOLDER}/extract/DDS3_full*merged.npz --nofulljones
#singularity exec -B ${SING_BIND} ${SING_IMAGE} --noprofile --norc killMS2H5parm.py lotss_smoothed.h5 ${FOLDER}/extract/DDS3_full*smoothed.npz --nofulljones
