...
class MergeH5:
    ...
    def _unpack_h5(self, st, solset, soltab):
        ...
        # Do checks
        if (self.ax_time[0] > time_axes[-1] or time_axes[0] > self.ax_time[-1]) and (len(time_axes)>1 and len(self.ax_time)>1):
        ...
        if (self.ax_freq[0] > freq_axes[-1] or freq_axes[0] > self.ax_freq[-1]) and (len(freq_axes)>1 and len(self.ax_freq)>1) and not self.freq_concat:
...
def parse_input():
    ...
    parser = ArgumentParser(description='h5parm merger script for merging h5parms containing phase and/or amplitude solutions for calibrating LOFAR observations')
